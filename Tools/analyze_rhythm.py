"""Analyze Unity-decoded user audio with numpy; estimates require listening review."""
import json
...
info = json.loads((out / 'alcatraz-info.json').read_text(encoding='utf-8-sig'))
audio = np.fromfile(out / 'alcatraz.f32', dtype='<f4').reshape(-1, info['channels']).mean(axis=1)
...
duration = min(64., float(info['duration']))
...
# Snap each predicted beat to a nearby detected onset, preserving the measured drift.
marks=[]
for predicted in np.arange(phase,float(info['duration'])-.1,period):
    ...
    else: picked=float(predicted)
    if not marks or picked>marks[-1]+.15: marks.append(round(picked,5))
report={'source':'User supplied El Alcatraz.mp3','duration':info['duration'],
...
(out/'alcatraz-analysis.json').write_text(json.dumps(report,indent=2),encoding='utf-8')
print(json.dumps({k:v for k,v in report.items() if k!='beatTimes'},indent=2))
print('Detected beat markers:',len(marks))
